...
This call provides the detailed output for a specified domain configured and associated 
with an account. This call is not capable of returning details for a domain that has been 
deleted.
...
The following examples show the default parameter settings ( ``showRecords`` = ``true &`` 
``showSubdomains`` = ``false`` ) for the List domain details call. This call returns 
information with records but no information about subdomains. Because these parameter values 
are the defaults, this call works the same way if both of the parameters are omitted.
